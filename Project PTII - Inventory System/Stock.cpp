...
void Stock::updateStock(std::string _branch, std::string _code, std::string _amount) {
	//To create file name based on the branch name
	std::string filename = _branch + ".csv";
	...
	int _available = 0;
	int _inventory_value = 0;
	int row_change = 0;
	for (int i = 0; i < row; i++) {
		if (code[i].compare(_code) == 1) {
			row_change = i;
...
void Stock::deleteItem(int index, std::string branch) {
	std::string filename = branch + ".csv";
